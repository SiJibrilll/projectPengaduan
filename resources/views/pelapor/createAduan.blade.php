...
<form class="mt-24" action="/aduan/store" method="POST" enctype="multipart/form-data">
    ...
    <input class="mt-4 w-full border-b-2 border-[#ABABAB] focus:border-gray-600" type="text" name="judul" id="" value="{{old('judul')}}" placeholder="Masukan inti aduan disini">
    @error('judul')
        <small class="text-red-500 text-xs mt-1">{{ $message }}</small>
    @enderror
    <h1 class="font-semibold text-[#585858] text-2xl mt-8">Deskripsi</h1>
    <div class="bg-white rounded-lg shadow-md p-4">
        <textarea name="deskripsi" placeholder="Deskripsikan aduan dengan lebih detail..." class="w-full h-32 border border-gray-300 rounded p-2 resize-none" placeholder="Enter your text...">{{old('deskripsi')}} </textarea>
    </div>
    @error('deskripsi')
        <small class="text-red-500 text-xs mt-1">{{ $message }}</small>
    @enderror
