<x-pelapor>
    <form class="mt-24" action="/aduan/update/{{$aduan->id}}" method="POST" enctype="multipart/form-data">
        ...
        <input class="mt-4 w-full border-b-2 border-[#ABABAB] focus:border-gray-600" type="text" name="judul" id="" placeholder="Masukan inti aduan disini" value="{{null == old('judul')? $aduan->judul : old('judul')}}">
        ...
        <div class="bg-white rounded-lg shadow-md p-4">
            <textarea name="deskripsi" placeholder="Deskripsikan aduan dengan lebih detail..." class="w-full h-32 border border-gray-300 rounded p-2 resize-none" placeholder="Enter your text...">{{null == old('deskripsi')? $aduan->deskripsi : old('deskripsi')}}</textarea>
        </div>
