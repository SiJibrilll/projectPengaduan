<x-admin>
 <h1 class="mt-5 font-bold text-[#585858] text-3xl mb-10">Edit Tanggapan</h1>

    <form class="mt-10" action="/tanggapan/update/{{$tanggapan->id}}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col sm:flex-row justify-between mr-10">
            ...
            <div class="flex grow flex-col ml-10">
                ...
                <select name="status"
                    ...
                    <option {{$item == $tanggapan->status ? 'selected' : ''}} value="{{ $index }}">
                        {{ ucfirst($item) }}
                    </option>
                    @endforeach
                </select>
        ...
        <input type="hidden" value="{{$tanggapan->aduan->id}}" name="aduan_id">
    </form>

    <script>
        function goBack() {
            window.location.href = '/aduan/show/' + @json($tanggapan->aduan->id);
        }
    </script>
</x-admin>